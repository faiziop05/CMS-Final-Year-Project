...
const CourseHistorySubject = ({ route, navigation }) => {
  ...
  const renderFunction = (item) => {
    ...
    const barColor =
      percentage >= 0.9
        ? "green"
        : percentage >= 0.85 && percentage < 0.90
          ? "lightgreen"
          : percentage >= 0.8 && percentage < 0.85
            ? "yellow"
            : percentage >= 0.75 && percentage < 0.80
              ? "gold"
              : percentage >= 0.7 && percentage < 0.75
                ? "gold"
                : percentage >= 0.65 && percentage < 0.70
                  ? "orange"
                  : percentage >= 0.60 && percentage < 0.65
                    ? "#8B8000"
                    : percentage >= 0.55 && percentage < 0.60
                      ? "#FF8000"
  ...
  const renderSectionHeader = ({ section }) => (
    <View style={{}}>
      <View style={{ width: "75%" }}>
        <Text style={styles.SectionsHeadings}>{section.title}</Text>
        <View
          style={{
            flexDirection: "row",
            marginLeft: 10,
  ...
  const renderSectionFooter = ({ section }) => {
    ...
    percentage >= 0.90
      ...
      : percentage > 0.60 && percentage < 0.65
      ? "#8B8000"
      : percentage === 0.6 // Added case for 60%
      ? "#8B8000"
      : percentage >= 0.55 && percentage < 0.60
      ? "#FF8000"
      : percentage >= 0.5 && percentage < 0.55
...
export default CourseHistorySubject;
