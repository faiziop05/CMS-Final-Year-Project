...
export const CoordinatorProfile = () => {
    ...
    if (coordinatorId) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/CoordinatorRoutes/get/${coordinatorId}`
        );
  ...
  const fetchProfilePicture = async () => {
    if (coordinatorId) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/profile/getprofilepicture/${coordinatorId}`,
          {
  ...
  const uploadProfilePicture = async () => {
    if (selectedImage && coordinatorId) {
      const formData = new FormData();
      formData.append("file", selectedImage);
      formData.append("userId", coordinatorId);
  ...
  const handleSaveChanges = async () => {
    ...
    if (!phoneNumberRegex.test(phoneNo)) {
      ...
      try {
        ...
        await axios.put(
          `http://localhost:3000/api/CoordinatorRoutes/updateCoordinatorProfile/${coordinatorId}`,
          updatedUser
        );
        setAdmin((prevAdmin) => ({
          ...prevAdmin,
    ...
    <div>
      ...
      <div className="ProfileContainer">
        <div className="ProfileFormContainer1">
          <h2 className="ProfileHeading">Coordinator Profile</h2>
