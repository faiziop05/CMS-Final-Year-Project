...
export const AdminProfile = () => {
    ...
    if (adminId) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/MainAdminConrollerRoutes/get/${adminId}`
        );
  ...
  const fetchProfilePicture = async () => {
    if (adminId) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/profile/getprofilepicture/${adminId}`,
          {
  ...
  const handleSaveChanges = async () => {
    ...
    if (!phoneNumberRegex.test(phoneNo)) {
      ...
      try {
        ...
        await axios.put(
          `http://localhost:3000/api/MainAdminConrollerRoutes/updateAdminProfile/${adminId}`,
          updatedUser
        );
        alert("Changes saved");
      } catch (error) {
    ...
    <div>
      ...
      <div className="ProfileContainer">
        <div className="ProfileFormContainer1">
          <h2 className="ProfileHeading">Admin Profile</h2>
